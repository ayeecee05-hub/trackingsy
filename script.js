...
// ── Pagination state ──────────────────────────────────────────────────────────
const PAGE_SIZE       = 10;   // cards per page — matches the 5-col × 2-row grid
let   currentPage     = 1;
...
/**
 * Renders exactly PAGE_SIZE slots on the current page.
 * If the last page has fewer real cards than PAGE_SIZE, invisible ghost
 * placeholder cards fill the remaining slots so the grid keeps its fixed
 * height and the Prev/Next bar stays pinned at the bottom — matching page 1.
 */
function renderPage() {
  ...
  const pageUsers = filteredBorrowers.slice(start, start + PAGE_SIZE);
  ...
  // Ghost placeholder cards to fill remaining slots → keeps grid same height
  const remainder = PAGE_SIZE - pageUsers.length;
  for (let i = 0; i < remainder; i++) {
    const ghost = document.createElement("div");
    ghost.className = "borrower-card borrower-card-ghost";
  ...
  if (pagination) {
    ...
    document.getElementById("pageInfo").textContent =
      `Page ${currentPage} of ${totalPages} (${total} user${total !== 1 ? "s" : ""})`;
